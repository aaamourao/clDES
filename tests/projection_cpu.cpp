...
main()
{
    ...
    cldes::DESystem<4u> supervisor = cldes::op::supC(plant, spec, non_contr);

    for (auto const s : supervisor.getMarkedStates()) {
        std::cout << s << " ";
    }
    std::cout << std::endl;

    cldes::EventsSet<4u> alpha;
    alpha.set();
    alpha.set(3, false);
    ...
    expected_result << "0 1 0 " << std::endl;
    expected_result << "0 0 4 " << std::endl;
    expected_result << "0 0 0 " << std::endl;
